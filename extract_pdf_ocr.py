...
def get_cached_images(output_dir, max_pages=None, start_page=1):
    ...
    if not image_paths:
        return []
    if start_page > 1:
        image_paths = [p for p in image_paths if int(p.stem.split("_")[-1]) >= start_page]
    if max_pages is not None:
        image_paths = image_paths[:max_pages]
    return [str(p) for p in image_paths]


def extract_text_with_ocr(pdf_path, languages=['si', 'en'], max_pages=None, start_page=1, dpi=200):
    """
    ...
    image_paths = get_cached_images(output_dir, max_pages=max_pages, start_page=start_page)
    if image_paths:
...
def extract_with_detailed_analysis(pdf_path, max_pages=10, start_page=1):
    ...
    image_paths = get_cached_images("pdf_images", max_pages=max_pages, start_page=start_page)
    ...
    try:
        reader = easyocr.Reader(['si', 'en'], gpu=False)
        ...
        for image_path in image_paths:
            ...
            for i, (bbox, text, confidence) in enumerate(results[:5], 1):  # Show first 5
...
def main():
    ...
    # Main OCR extraction
    extract_text_with_ocr(pdf_path, max_pages=max_pages, start_page=start_page, dpi=dpi)
    
    # Detailed analysis
    extract_with_detailed_analysis(pdf_path, max_pages=5, start_page=start_page)
